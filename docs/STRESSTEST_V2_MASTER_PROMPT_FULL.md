...
---
## ✅ [2026-04-21] CogsBlock mean-reversion фикс (Rusal)

### Проблема
CogsBlock использовал cumulative macro drift:
`cogs_ratio = anchor * macro_ratio` → за 5 лет COGS% рос 77→85% (+8pp)
EBITDA margin падала 8.6% → 0.7% (нереалистично для commodity producer)

### Фикс
Mean-reversion: `cogs_ratio = anchor * (1 + deviation * dampening)`
dampening=0.30, clamp=anchor±0.06 (1σ)
Все параметры вынесены в YAML (cogs.mean_reversion_dampening, cogs.clamp_sigma)

### Результат
EBITDA% 2026: 10.7% | 2030: 8.1% | BS=0 | US Steel не затронут
